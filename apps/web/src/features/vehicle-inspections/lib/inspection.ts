...
/** Informational hint only — the form does not block entry outside this window. */
export const formatEarliestScheduleHint = (
  type: VehicleInspectionType,
  expiresAt: string,
): string | null => {
  const earliest = earliestScheduleDate(type, expiresAt);

  return earliest ? `Sledeći se može zakazati od ${formatDate(earliest)}.` : null;
};
